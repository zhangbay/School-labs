...
private:
   ...
   int storedCount;
   bool insert_bst(TreeNode *ins, TreeNode *curr);//insert in binary search tree
   DBentry * searchTree(TreeNode *curr ,string name);
   void removing(TreeNode * & curr, string name);//remove single leaf
   DBentry * Maximum(TreeNode *curr);// find the maximum leaf for replacement
   void delete_tree(TreeNode * myroot);//delete whole tree
   void activesearch(TreeNode * searchnode, int & counts)const;//search for active status in leaves
